...
$(document).ready(function () {
    ...
    $(invoiceAnchor).attr("href", "#tab1").text("Invoice");
    ...
    $(flbAnchor).attr("href", "#tab2").text("Closedown");
    ...
    $("#SelectedQueryReason").change(function () {
        ...
        if (selectedVal === "Other") {
            $(".invoice-query-other").show(500);
        } else {
            $(".invoice-query-other").hide();
        }
    });
    ...
    $("#SaveQuery").on("click", function () {
        ...
        if (selectedVal) {
            ...
            $.ajax({
                type: "POST",
                url: "/InvoiceApproval/QueryInvoice",
                content: "application/json; charset=utf-8",
                dataType: "json",
                data: { Id: $("#InvoiceId").val(), Notes: queryText },
                success: function(d) {
                    if (d === "Error") {
                        $("#QueryError").show();
                    } else {
    ...
    $("#SaveRejection").on("click", function () {
        ...
        var notes = $("#RejectionNotes").text();
        if (selectedVal) {
            // InvoiceId, InvoiceHeader.Id, SelectedRejectionReason, RejectionNotes
            $.ajax({
                type: "POST",
                url: "/InvoiceApproval/RejectFirstApproval",
                content: "application/json; charset=utf-8",
                dataType: "json",
                data: { Id: $("#InvoiceId").val(), SelectedRejectionReason: selectedVal, RejectionNotes: notes },
                success: function (d) {
                    if (d === "Error") {
                        $("#RejectError").show();
                    } else {
    ...
    $("#RejectInvoice, #QueryInvoice, #ViewInvoiceLines").on("click", function () {
